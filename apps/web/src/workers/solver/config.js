...
const QUALITY_ORDER = ["high-fidelity", "balanced", "fast-preview"];
export function resolveQualityProfile(requested, estimatedBytes) {
    const warnings = [];
    ...
    if (chosen.id === requested) {
        ...
        if (requested !== fallback.id) {
            warnings.push(`Quality automatically downgraded to ${fallback.id}; requested ${requested} exceeded safe memory budget.`);
            chosen = fallback;
        }
    }
    return { profile: chosen, warnings };
}
export function qualityProfileConfig(profile) {
