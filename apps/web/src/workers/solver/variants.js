...
export function variantParams(index, count, minThickness, jitterSeed = 0) {
...
function iouWithStride(a, b, domainMask, stride) {
    let inter = 0;
    let union = 0;
...
export function computeSignature(occupancy, domainMask) {
    let count = 0;
    let sx = 0;
    let sy = 0;
    let sz = 0;
...
export function isUniqueVariant(candidateOccupancy, candidateSignature, existing, domainMask) {
    for (const other of existing) {
        ...
        const volumeRatio = Math.min(candidateSignature.volumeRatio, other.signature.volumeRatio) /
