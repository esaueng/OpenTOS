...
function cubePositions(size = 1) {
    const geometry = new THREE.BoxGeometry(size, size, size).toNonIndexed();
    return geometry.getAttribute("position").array.slice();
}
describe("solver core", () => {
    ...
    it("preserved extraction keeps exact triangles", () => {
        ...
        const arr = preserved.getAttribute("position").array;
    ...
    it("influence field is stronger near force axis than far region", () => {
        ...
        const base = computeBaseInfluenceFields(grid, domainMask, preserveMask, preservedDistance, forceDistance, [
            {
                point: [0.55, 0, 0],
                direction: [-1, 0, 0],
                magnitudeN: 100
