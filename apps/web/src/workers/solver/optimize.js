...
function neighborSmooth(src, domainMask, grid, factor) {
...
function thresholdForVolume(values, domainMask, preserveMask, keepFraction) {
    const bins = 256;
...
function enforceThickness(occupancy, domainMask, preserveMask, grid, minThickness) {
    if (minThickness <= 0) {
...
function smoothDensityField(src, domainMask, grid, iterations) {
    let field = src;
...
function carveInteriorVoids(occupancy, influence, domainMask, preserveMask, grid, aggressiveness) {
...
function openExteriorWindows(occupancy, influence, surfaceDistance, domainMask, preserveMask, anchorMask, grid, aggressiveness) {
    const opened = occupancy.slice();
    const passes = Math.max(1, Math.min(3, Math.round(1 + aggressiveness * 2)));
    for (let pass = 0; pass < passes; pass += 1) {
        const next = opened.slice();
        const shellBand = 1.2 + pass * 0.8;
        const threshold = 0.46 + aggressiveness * 0.2 + pass * 0.05;
        for (let idx = 0; idx < opened.length; idx += 1) {
            if (!opened[idx] || !domainMask[idx] || preserveMask[idx] || anchorMask[idx]) {
                continue;
            }
            if (surfaceDistance[idx] > shellBand) {
                continue;
            }
            const { x, y, z } = decodeIndex(grid, idx);
            let outsideNeighbors = 0;
            ...
            for (const [dx, dy, dz] of NEIGHBORS) {
                ...
                const nIdx = index3D(grid, nx, ny, nz);
                if (!domainMask[nIdx] || !opened[nIdx]) {
                    outsideNeighbors += 1;
                }
                else {
                    occupiedNeighbors += 1;
            ...
            if (outsideNeighbors < 1) {
                continue;
            }
            if (occupiedNeighbors >= 3 && influence[idx] < threshold) {
                next[idx] = 0;
            }
        }
        opened.set(next);
...
function stripOuterShell(occupancy, influence, surfaceDistance, domainMask, preserveMask, anchorMask, grid, aggressiveness) {
    const stripped = occupancy.slice();
    const passes = Math.max(1, Math.min(2, Math.round(1 + aggressiveness)));
    for (let pass = 0; pass < passes; pass += 1) {
        const next = stripped.slice();
        const shellBand = 1.4 + pass * 0.7;
        const threshold = 0.48 + aggressiveness * 0.18 + pass * 0.05;
        for (let idx = 0; idx < stripped.length; idx += 1) {
            if (!stripped[idx] || !domainMask[idx] || preserveMask[idx] || anchorMask[idx]) {
                continue;
            }
            if (surfaceDistance[idx] > shellBand) {
                continue;
            ...
            for (const [dx, dy, dz] of NEIGHBORS) {
                ...
                const nIdx = index3D(grid, nx, ny, nz);
                if (!domainMask[nIdx] || !stripped[nIdx]) {
                    outsideNeighbors += 1;
                }
            }
            if (outsideNeighbors >= 1 && influence[idx] < threshold) {
                next[idx] = 0;
            }
        }
        stripped.set(next);
...
function pruneToVolumeTarget(occupancy, influence, surfaceDistance, domainMask, preserveMask, anchorMask, grid, targetVolumeFraction, voidBias) {
    ...
    for (let pass = 0; pass < passes && occupied > targetOccupied; pass += 1) {
        ...
        for (let idx = 0; idx < pruned.length; idx += 1) {
            ...
            const shellDepth = Math.min(surfaceDistance[idx], 6) / 6;
            candidates.push({
                idx,
                score: influence[idx] * 0.72 + neighborSupport * 0.14 + shellDepth * 0.14
            });
        }
...
function retainConnectedToAnchors(occupancy, domainMask, preserveMask, anchorMask, grid) {
...
export function solveDensityField({ grid, domainMask, preserveMask, anchorMask, influence, targetVolumeFraction, iterations, smoothFactor, minThickness, voidBias, surfaceDistance }) {
    ...
    for (let i = 0; i < rho.length; i += 1) {
        const shellThresholdBoost = surfaceDistance[i] <= 1.6 ? 0.04 + voidBias * 0.05 : 0;
        const occupancyThreshold = 0.52 + shellThresholdBoost;
        const shellProtected = surfaceDistance[i] > 1.4 || influence[i] > 0.44 + voidBias * 0.08 || anchorMask[i];
        if (domainMask[i] && (preserveMask[i] || (rho[i] >= occupancyThreshold && shellProtected))) {
            occupancy[i] = 1;
        }
    }
    const carveAggressiveness = clamp01((0.38 - targetVolumeFraction) * 3.2 + voidBias * 0.4);
    const interiorCarved = carveInteriorVoids(occupancy, influence, domainMask, preserveMask, grid, carveAggressiveness);
    const openedShell = openExteriorWindows(interiorCarved, influence, surfaceDistance, domainMask, preserveMask, anchorMask, grid, carveAggressiveness);
    const pruned = pruneToVolumeTarget(openedShell, influence, surfaceDistance, domainMask, preserveMask, anchorMask, grid, targetVolumeFraction, voidBias);
    ...
    const shellStripped = stripOuterShell(thicknessEnforced, influence, surfaceDistance, domainMask, preserveMask, anchorMask, grid, carveAggressiveness);
    const finalOccupancy = retainConnectedToAnchors(shellStripped, domainMask, preserveMask, anchorMask, grid);
