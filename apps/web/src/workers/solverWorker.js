...
import { buildVoxelGrid, computeBoundingBox, estimateSolverMemoryBytes, faceCountFromPositions, faceCenter, getFaceSet, getPreservedData } from "./solver/geometry";
...
function buildTrussBoostField(grid, domainMask, surfaceDistance, preservedTargets, forces) {
...
async function solveInWorker(payload) {
    ...
    const designMask = !designFaceSet.size || designCoverage > 0.55
        ? domainMask.slice()
    ...
    const baseFields = computeBaseInfluenceFields(grid, constrainedDomain, preserveMask, preservedDistance, forceDistance, forces, preservedTargets, qualityConfig.connectivityIterations);
    const outcomes = [];
    ...
    while (outcomes.length < request.targets.outcomeCount && attempt < maxAttempts) {
        ...
        for (let i = 0; i < influence.length; i += 1) {
            ...
            const protectedBand = Math.max(Math.exp(-preservedDistance[i] * 0.45), Math.exp(-forceDistance[i] * 0.45));
        ...
        const generatedGeometry = trussBoostField && preservedTargets.length >= 3
            ...
            : extractIsoSurface(grid, densityResult.density, constrainedDomain, 0.52, qualityConfig.taubinIterations);
