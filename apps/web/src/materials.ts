import type { StudyCreateRequest } from "@contracts/index";

export type MaterialName = StudyCreateRequest["material"];

export interface MaterialProperties {
  densityKgM3: number;
  elasticModulusGPa: number;
  yieldStrengthMPa: number;
}

export const MATERIAL_OPTIONS: MaterialName[] = [
  "Aluminum 6061",
  "PLA",
  "PETG",
  "ABS",
  "ASA",
  "Nylon (PA12)",
  "Polycarbonate (PC)"
];

export const MATERIAL_PROPERTIES: Record<MaterialName, MaterialProperties> = {
  "Aluminum 6061": {
    densityKgM3: 2700,
    elasticModulusGPa: 69,
    yieldStrengthMPa: 276
  },
  PLA: {
    densityKgM3: 1240,
    elasticModulusGPa: 3.5,
    yieldStrengthMPa: 60
  },
  PETG: {
    densityKgM3: 1270,
    elasticModulusGPa: 2.1,
    yieldStrengthMPa: 50
  },
  ABS: {
    densityKgM3: 1040,
    elasticModulusGPa: 2.1,
    yieldStrengthMPa: 40
  },
  ASA: {
    densityKgM3: 1070,
    elasticModulusGPa: 2.0,
    yieldStrengthMPa: 42
  },
  "Nylon (PA12)": {
    densityKgM3: 1010,
    elasticModulusGPa: 1.7,
    yieldStrengthMPa: 45
  },
  "Polycarbonate (PC)": {
    densityKgM3: 1200,
    elasticModulusGPa: 2.3,
    yieldStrengthMPa: 65
  }
};
